        ...
        <div class="estrutura">
            ...
            <div class="conteudo">
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    ...
                    // prepara as variáveis que vieram da chamada (segurança)
                    $descricao = (isset($_POST['descricao']) ? strip_tags( trim( $_POST['descricao']) ) : '');
                    $raca_id = (isset($_POST['raca_id']) ? strip_tags( trim( $_POST['raca_id']) ) : '');
                    
                    // executa a atualização dos dados
                    TDBConnection::beginTransaction();
                    TDBConnection::prepareQuery("UPDATE `racas` SET descricao = :descricao WHERE `id` = :id;");
                    ...
                    header("Location: formAlterar.php?id=$raca_id");
                    exit;
                }
                
                // consulta os dados
                TDBConnection::prepareQuery("SELECT * FROM racas where id = :id order by descricao;");
                ...
                <form name="formExec" id="formExec" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                    <fieldset>
                        <input name="raca_id" type="hidden" id="raca_id"
                               value="<?php echo (isset($result->id) ? $result->id : "0"); ?>">

                        <label for="descricao">Descrição:</label>
                        <input type="text" name="descricao" id="descricao"
                               maxlength="30" size="20" required autofocus
